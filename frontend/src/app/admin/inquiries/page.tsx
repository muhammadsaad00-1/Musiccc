...
const statusConfig = {
    ...
    booked: { label: 'Booked', color: 'bg-green-500/20 text-green-400 border-green-500/30', icon: CheckCircle },
...
export default function InquiriesPage() {
    ...
    const fetchRequirements = async () => {
        ...
        try {
            const url = filter === 'all'
                ? 'http://localhost:8001/api/requirements'
                : `http://localhost:8001/api/requirements?status=${filter}`;
    ...
    const updateStatus = async (id: string, newStatus: string) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:8001/api/requirements/${id}/status`, {
                method: 'PUT',
                body: formData
            });
            ...
            if (response.ok) {
                ...
                if (selectedRequirement?.id === id) {
                    setSelectedRequirement({ ...selectedRequirement, status: newStatus as any });
                }
            }
        } catch (error) {
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                {loading ? (
                    ...
                    <div className="text-center py-20">
                        ...
                        <p className="text-gray-500">Booking requests will appear here when customers submit them.</p>
                    </div>
                ) : (
                            ...
                            <table className="w-full">
                                ...
                                <tbody className="divide-y divide-gray-800">
                                    {requirements.map((req) => {
                                        ...
                                        return (
                                            <tr key={req.id} className="hover:bg-[#222] transition-colors">
                                                ...
                                                <td className="px-6 py-4">
                                                    <span className="capitalize text-gray-300">{req.artist_type}</span>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <span className="text-gray-300">{formatBudget(req.budget)}</span>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border ${statusConfig[req.status]?.color || ''}`}>
                                                ...
                                                <td className="px-6 py-4">
                                                    <div>
                                                        <p className="text-white">{formatDate(req.event_date)}</p>
                                                        <p className="text-xs text-gray-500">Received: {formatDate(req.created_at)}</p>
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <button
                ...
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-[#1a1a1a] rounded-2xl border border-gray-800 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="flex items-center justify-between p-6 border-b border-gray-800">
                            <div>
                                <h2 className="text-xl font-bold text-white">Booking Request Details</h2>
                                <p className="text-sm text-gray-500">ID: {selectedRequirement.id}</p>
                            </div>
                            <button
                        ...
                        <div className="p-6 space-y-6">
                            {/* Status Badge */}
                            <div className="flex items-center justify-between">
                                <span className={`inline-flex items-center gap-2 px-4 py-2 rounded-full font-medium border ${statusConfig[selectedRequirement.status]?.color || ''}`}>
                                    {(() => {
                                        const Icon = statusConfig[selectedRequirement.status]?.icon || Clock;
                                        return <Icon className="w-4 h-4" />;
                                    })()}
                                    {statusConfig[selectedRequirement.status]?.label || selectedRequirement.status}
                                </span>
                            ...
                            <div className="bg-[#0f0f10] rounded-xl p-5 border border-gray-800">
                                ...
                                <div className="grid gap-3">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-white">{selectedRequirement.customer_name}</span>
                                    </div>
                                    <div className="flex items-center gap-3">
                                        <Mail className="w-4 h-4 text-gray-500" />
                                        <a href={`mailto:${selectedRequirement.customer_email}`} className="text-blue-400 hover:underline">
                                            {selectedRequirement.customer_email}
                                        </a>
                                    </div>
                                    <div className="flex items-center gap-3">
                                        <Phone className="w-4 h-4 text-gray-500" />
                                        <a href={`tel:${selectedRequirement.customer_phone}`} className="text-blue-400 hover:underline">
                            ...
                            <div className="bg-[#0f0f10] rounded-xl p-5 border border-gray-800">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <p className="text-white font-medium">{selectedRequirement.event_type}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs text-gray-500 mb-1">Event Date</p>
                                        <p className="text-white font-medium">{formatDate(selectedRequirement.event_date)}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs text-gray-500 mb-1">Location</p>
                                        <p className="text-white font-medium">{selectedRequirement.event_location}</p>
                                    </div>
                                    <div>
                                        <p className="text-xs text-gray-500 mb-1">Artist Type</p>
                                        <p className="text-white font-medium capitalize">{selectedRequirement.artist_type}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-[#0f0f10] rounded-xl p-5 border border-gray-800">
                                ...
                                <div className="space-y-3">
                                    <div>
                                        ...
                                        <p className="text-white font-medium">{formatBudget(selectedRequirement.budget)}</p>
                                    </div>
                                    {selectedRequirement.message && (
                                        <div>
                                            <p className="text-xs text-gray-500 mb-1">Additional Notes</p>
                                            <p className="text-gray-300 italic">{selectedRequirement.message}</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {Object.entries(statusConfig).map(([status, config]) => {
                                        ...
                                        const isUpdating = updatingStatus === selectedRequirement.id;

                                        return (
                                            <button
                                                key={status}
                                                onClick={() => updateStatus(selectedRequirement.id, status)}
