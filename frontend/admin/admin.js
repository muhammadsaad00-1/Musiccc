...
function closeModal(modalId) {
  ...
  if (modalId === "performerModal") {
    ...
    modalTitle.innerHTML =
      '<i class="fas fa-microphone"></i> Add New Performer';
...
function initFilePreview() {
  ...
  // Performer header image preview
  const performerHeaderImageInput = document.getElementById(
    "performerHeaderImage",
  );
  if (performerHeaderImageInput) {
    performerHeaderImageInput.addEventListener("change", (e) => {
      handleImagePreview(e.target, "performerHeaderImagePreview");
    });
  }

  // Performer gallery images preview
  const performerGalleryImagesInput = document.getElementById(
    "performerGalleryImages",
  );
  if (performerGalleryImagesInput) {
    performerGalleryImagesInput.addEventListener("change", (e) => {
      handleMultipleImagePreview(e.target, "performerGalleryImagesPreview");
    });
  }
...
async function loadPerformersForEventDropdown() {
  try {
    ...
    if (performers.length === 0) {
      select.innerHTML =
        '<option value="">No performers available - add performers first</option>';
    } else {
      select.innerHTML = `
...
document
  ...
  .addEventListener("submit", async (e) => {
    ...
    try {
      ...
      if (locationsValue) {
        const locationsArray = locationsValue
          .split(",")
          .map((l) => l.trim())
          .filter((l) => l);
      ...
      const popularSongsValue = formData.get("popular_songs");
      if (popularSongsValue) {
        const popularSongsArray = popularSongsValue
          .split(",")
          .map((s) => s.trim())
          .filter((s) => s);
        formData.set("popular_songs", JSON.stringify(popularSongsArray));
      }
      ...
      if (response.ok) {
        ...
        showToast(
          `Failed to save performer: ${error.detail || "Unknown error"}`,
          "error",
        );
      }
    } catch (error) {
      ...
      submitBtn.innerHTML = editingPerformerId
        ? '<i class="fas fa-save"></i> Update Performer'
        : '<i class="fas fa-save"></i> Add Performer';
      editingPerformerId = null;
    }
  });
...
function editPerformer(id) {
  ...
  form.querySelector('[name="youtube_url"]').value =
    performer.youtube_url || "";
  form.querySelector('[name="locations"]').value = performer.locations
    ? performer.locations.join(", ")
    : "";
  form.querySelector('[name="genres"]').value = performer.genres
    ? performer.genres.join(", ")
    : "";
  form.querySelector('[name="videos"]').value = performer.videos
    ? performer.videos.join(", ")
    : "";
  form.querySelector('[name="popular_songs"]').value = performer.popular_songs
    ? performer.popular_songs.join(", ")
    : "";
  ...
  if (performer.profile_image_url) {
    preview.innerHTML = `<img src="${performer.profile_image_url}" alt="Profile Image">`;
  }

  // Show current header image preview if exists
  const headerPreview = document.getElementById("performerHeaderImagePreview");
  if (performer.header_image_url) {
    headerPreview.innerHTML = `<img src="${performer.header_image_url}" alt="Header Image">`;
  }

  // Show current gallery images preview if exists
  const galleryPreview = document.getElementById(
    "performerGalleryImagesPreview",
  );
  if (performer.gallery_image_urls && performer.gallery_image_urls.length > 0) {
    galleryPreview.innerHTML = performer.gallery_image_urls
      .map(
        (url) =>
          `<img src="${url}" alt="Gallery Image" style="width: 100px; height: 100px; object-fit: cover; margin: 5px; border-radius: 8px;">`,
      )
      .join("");
  }
...
document.getElementById("eventForm").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const error = await response.json();
      showToast(
        `Failed to add event: ${error.detail || "Unknown error"}`,
        "error",
      );
    }
  } catch (error) {
...
function viewRequest(id) {
  ...
  document.getElementById("emailLink").href =
    `mailto:${selectedRequest.customer_email}?subject=Re: Your Booking Request for ${selectedRequest.event_type}`;
  document.getElementById("phoneLink").href =
    `tel:${selectedRequest.customer_phone}`;
...
async function updateRequestStatus(status) {
  ...
  try {
    ...
    const response = await fetch(
      `${API_BASE}/api/requirements/${selectedRequest.id}/status`,
      {
        method: "PUT",
        body: formData,
      },
